...
import json
from typing import Literal, Any, Dict, List, Union, Type, Optional
...
class VectorSearchRetrievalToolInput(BaseModel):
    """Input schema for VectorSearchRetrievalTool."""
    query: str = Field(..., description="The question or query input from the user.")
    filters: Optional[FilterDict] = Field(default = None, description="Filters to apply to the search.")

class VectorSearchRetrievalTool(BaseTool):
    ...
    name: str
    description: str
    args_schema: Type[BaseModel] = VectorSearchRetrievalToolInput
    ...
    @mlflow.trace(span_type="TOOL", name="retriever")
    def _run(self, query: str, filters: FilterDict = None) -> str:
        """
        ...
        docs = self.convert_vector_search_to_documents(
            results.as_dict(), self.doc_similarity_threshold
        )

        return self.convert_documents_to_string(docs)

    @mlflow.trace(span_type="PARSER")
    def convert_documents_to_string(self, docs):
        """Convert the retrieved documents to a combined strings"""
...
#     def _run(self, query: str, filters: Dict[Any, Any] = None) -> List[Document]:
#         """
#         Performs vector search to retrieve relevant chunks.
